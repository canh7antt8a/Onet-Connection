	...
	public void unlock(final int score){
		mActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if(gameHelper.isSignedIn()){
					if(score == 1){
						Games.Achievements.unlock(gameHelper.getApiClient(), "CgkIh8Pc0rUbEAIQAA");
					}
					if(score == 4){
						Games.Achievements.unlock(gameHelper.getApiClient(), "CgkIh8Pc0rUbEAIQAQ");
					}
					if(score == 8){
						Games.Achievements.unlock(gameHelper.getApiClient(), "CgkIh8Pc0rUbEAIQAg");
					}
					if(score == 12){
						Games.Achievements.unlock(gameHelper.getApiClient(), "CgkIh8Pc0rUbEAIQAw");
					}
					if(score == 16){
						Games.Achievements.unlock(gameHelper.getApiClient(), "CgkIh8Pc0rUbEAIQBA");
					}
